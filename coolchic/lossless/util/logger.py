...
class TrainingLogger:
    def __init__(
        ...
        results_log_file_name = (
            f"{timestamp_string(self.start_time)}_coolchic_{self.image_name}.log"
        )
        self.train_log_file_path = os.path.join(self.train_logs_path, train_log_file_name)
        self.results_log_file_path = os.path.join(self.results_logs_path, results_log_file_name)
        with open(self.results_log_file_path, "w") as f:
            f.write(start_print)
            f.write(f"Log file created at {timestamp_string(self.start_time)}\n")
    ...
    def save_model(self, model, total_bpd: float):
        ...
        if not self.write_to_disk:
            return
        model_file_name = f"{timestamp_string(self.start_time)}_trained_coolchic_{self.image_name}_img_rate_{total_bpd}.pth"
        model_file_path = os.path.join(self.trained_models_path, model_file_name)
        torch.save(model.state_dict(), model_file_path)
        self.log_result(f"Model saved at {model_file_path} with rate {total_bpd} bpd.")
        return
